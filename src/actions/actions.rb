module Actions
  ...
  def self.generate_food(state)
    #rand
    coords = Model::Coord.new(rand(state.grid.rows), rand(state.grid.cols))
    while position_is_in_snake?(state, coords) 
      
      coords = Model::Coord.new(rand(state.grid.rows), rand(state.grid.cols))
     
    end
    new_food = Model::Food.new(coords.row, coords.col)
    state.food = new_food
    state
  end
    
  def self.position_is_in_snake?(state, food)
    state.snake.positions.include? food
  end
